...
// First 7 are Spark-specific and shown by default.
// The remaining are SEO keyword questions, revealed with "Show more".
const FAQS = [
  {
...
export default function FAQ() {
  ...
  function handleShowLess() {
    setShowAll(false);
    // Close any expanded item that's now hidden
    if (open !== null && open >= INITIAL_COUNT) setOpen(null);
  }

  const visible = FAQS.slice(0, INITIAL_COUNT);
  const hidden   = FAQS.slice(INITIAL_COUNT);
    ...
    <section id="faq" className="bg-black">
      <div className="max-w-[1320px] mx-auto px-[5vw] py-16 md:py-[7rem]">
        ...
        <div className="border-t border-dark-gray">
          ...
          {visible.map((faq, i) => (
            <FAQItem key={i} faq={faq} index={i} open={open} onToggle={handleToggle} />
          ))}

          {/* Expandable extra questions */}
          <AnimatePresence initial={false}>
            {showAll && (
              <motion.div
                key="extra"
              ...
              >
                {hidden.map((faq, j) => {
                  ...
                  return (
                    ...
                    >
                      <FAQItem faq={faq} index={i} open={open} onToggle={handleToggle} />
                    </motion.div>
                  );
                })}
              </motion.div>
          ...
          {!showAll ? (
            <button
              ...
              className={[
                ...
                "font-condensed text-[0.82rem] tracking-[0.15em] uppercase",
                "px-8 py-4 border border-dark-gray text-offwhite",
            ...
            >
              Show Less
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true" style={{ transform: "rotate(45deg)" }}>
                <path d="M12 5v14M5 12h14" />
              </svg>
            </button>
          )}

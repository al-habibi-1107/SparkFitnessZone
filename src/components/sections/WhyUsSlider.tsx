...
export default function WhyUsSlider() {
  ...
  // Auto-advance — restarts whenever active or paused changes
  useEffect(() => {
    if (paused) return;
    const id = setTimeout(() => {
      setDir(1);
  ...
  return (
    ...
    >
      <div className="max-w-[1320px] mx-auto px-[5vw] py-[7rem]">
        ...
        {/* ── Main slide area ──────────────────────────────────── */}
        {/*
          Mobile:  flex-col — image on top (h-[280px]), text below
          Desktop: grid 50/50 — text left, image right
          The `order-*` classes handle the swap.
        */}
